...
public class DialectH2Test {
  ...
  @Test
  public void testRenameColumnWithConstraint() throws SQLException, OrmException {
    execute("CREATE TABLE ACCOUNTS (ACCOUNT_ID INT DEFAULT 0 NOT NULL, SHOW_USERNAME_IN_REVIEW_CATEGORY CHAR(1) DEFAULT 'N' NOT NULL)");
    execute("ALTER TABLE ACCOUNTS ADD CONSTRAINT SHOW_USERNAME_IN_REVIEW_CHECK CHECK (SHOW_USERNAME_IN_REVIEW_CATEGORY IN ('Y', 'N'))");
    execute("ALTER TABLE ACCOUNTS DROP CONSTRAINT SHOW_USERNAME_IN_REVIEW_CHECK");
    execute("ALTER TABLE ACCOUNTS ALTER COLUMN SHOW_USERNAME_IN_REVIEW_CATEGORY RENAME TO SHOW_USER_IN_REVIEW");
    execute("ALTER TABLE ACCOUNTS ADD CONSTRAINT SHOW_USER_IN_REVIEW_CHECK CHECK (SHOW_USER_IN_REVIEW IN('Y', 'N'))");
  }
}
