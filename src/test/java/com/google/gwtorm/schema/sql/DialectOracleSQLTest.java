...
public class DialectOracleSQLTest {
  ...
  @Test
  public void createTableWithTooLongColumn() throws OrmException, SQLException {
    assertTrue(dialect.listSequences(db).isEmpty());

    String column = "VERY_VERY_VERY_VERY_VERY_LONG_COLUMN_NAME";
    column = Util.makeSqlFriendly(column);
    assertEquals("very_very_very_very_v8859994c", column);
    execute("CREATE TABLE foo ("  + column + " INT)");
